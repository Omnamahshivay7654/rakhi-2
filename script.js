...
const musicStateKey = 'rakshaBandhanMusicState';
...
function initAudioEngine() {
  ...
  const savedMusicState = JSON.parse(localStorage.getItem(musicStateKey) || '{}');

  if (bgAudio) {
    bgAudio.volume = typeof savedMusicState.volume === 'number' ? savedMusicState.volume : 0.7;
    if (volumeSlider) volumeSlider.value = bgAudio.volume;
    if (typeof savedMusicState.currentTime === 'number') {
      bgAudio.addEventListener('loadedmetadata', () => {
        bgAudio.currentTime = Math.min(savedMusicState.currentTime, bgAudio.duration || savedMusicState.currentTime);
      }, { once: true });
    }
  }
  ...
  if (bgAudio) {
    bgAudio.addEventListener('timeupdate', saveMusicState);
    window.addEventListener('pagehide', saveMusicState);
  }

  const resumeSavedMusic = () => {
    if (savedMusicState.isPlaying && bgAudio && bgAudio.paused) {
      bgAudio.play().then(() => {
        isMusicPlaying = true;
        updateMusicUI(true);
      }).catch(() => {
        // Browsers may require a gesture again after a page navigation.
      });
    }
  };
